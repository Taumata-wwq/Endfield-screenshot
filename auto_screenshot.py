...
VALIDATION_RANGES = {
    "overlap": (0, 100),
    "capture_region": (0, 100),
    "grid": (1, MAX_GRID_SIZE),
}
...
CONFIG = {
    "stabilize_delay": 0.05,
    "screenshot_delay": 0.01,
    "capture_region_x": 0.626,
    "capture_region_y": 0.648,
    "capture_offset_y": 0.03,
    "drag_margin": 2,
    "drag_duration": 0.01,
    "base_window_size": (1600, 900),
    "output_folder": os.path.dirname(sys.executable if getattr(sys, 'frozen', False) else __file__),
    "output_format": "JPG",
}
...
class GameScreenshotTool:
    ...
    def move_camera(self, direction: str) -> None:
        ...
        time.sleep(rand_delay(0.05))
        pyautogui.mouseDown(button='middle')
        time.sleep(rand_delay(0.1))
...
class ScreenshotGUI:
    ...
    def _update_drag_display(self, keep_editable: bool = False) -> None:
        ...
        final_keep_editable = keep_editable or is_custom_region
    ...
    def _update_overlap_display(self) -> None:
        ...
        is_custom_region = hasattr(self, 'region_var') and self.region_var.get() == "自定义"
        self._update_entry(self.overlap_x_entry, f"{overlap_x * 100:.1f}", keep_editable=is_custom_region)
        self._update_entry(self.overlap_y_entry, f"{overlap_y * 100:.1f}", keep_editable=is_custom_region)
    ...
    def _on_scroll_mode_change(self, event=None) -> None:
        ...
        is_custom_aspect = self.aspect_ratio_var.get() == "自定义"
        is_custom_region = self.region_var.get() == "自定义"
        keep_editable = is_custom_aspect or is_custom_region
        self._update_drag_display(keep_editable=keep_editable)
        self._update_overlap_display()
        self._update_grid_display()
        if is_custom_region:
            self.rows_entry.config(state=tk.NORMAL)
    ...
    def _start_capture(self) -> None:
        ...
        if region == "自定义":
            try:
                ...
                if custom_rows > MAX_GRID_SIZE or custom_cols > MAX_GRID_SIZE:
                    return messagebox.showerror("错误", f"行列数最大为{MAX_GRID_SIZE}")
            except ValueError:
